...
def balanced_accuracy(y_pred, y_true):
    ...
    acc = class_accs(y_pred, y_true)
    return np.sum(acc) / len(acc)


def grid_search(X_grid, y_grid, model, model_name):
    """
    Tuning the hyperparameters by doing a grid search

    :param X_grid: Features of the dataset that are labeled up to now
    :param y_grid: Classes of the dataset that are labeled up to now
...
def get_clf_after_gridsearch(X_grid, y_grid, y_train, model, model_name):
    """
    Get the clf with the best parameters after doing a gridsearch

    :param X_grid: Features of the dataset that are labeled up to now
    :param y_grid: Classes of the dataset that are labeled up to now
    :param y_train: True y values in the training set, only used to determine the classes
    :param model: classification model
    :param model_name: Name of the classification model
    :return: The best parameters
    """

    best_param = grid_search(X_grid, y_grid, model, model_name)

    if model_name == "LogisticRegression":
        clf = SklearnClassifier(LogisticRegression(penalty = best_param["penalty"], 
            C = best_param["C"]), classes=np.unique(y_train))
    if model_name  == "RandomForest":
        clf = SklearnClassifier(RandomForestClassifier(criterion = best_param["criterion"], 
            max_depth = best_param["max_depth"]), classes=np.unique(y_train))
    if model_name == "SVM":
        clf = SklearnClassifier(SVC(C = best_param["C"], kernel = best_param["kernel"], 
            probability=True), classes=np.unique(y_train))
    if model_name == "DecisionTree":
        clf = SklearnClassifier(DecisionTreeClassifier(criterion = best_param["criterion"], 
            max_depth = best_param["max_depth"]), classes=np.unique(y_train))   

    return clf


def ActiveLearning_CrossValidation(X, y_true, folds, AL_method, n_cycles, batch_size, clf, classifier,
        n=10, initialtunings=False):
    """
    ...
    Default is 10.
    :param initialtunings: Optional parameter indicating if there has to be tuned the first n cycles.
    ...
    for i, (train_index, test_index) in enumerate(skf.split(X,y_true)):
        ...
        cycle, accuracies, indep_accuracies = AL_method(X_train_scaled, X_test_scaled, y_train, 
                y_test, n_cycles, batch_size, clf, classifier, n=n, initialtunings=initialtunings)
        acc.append(indep_accuracies)
        
    return cycle, np.mean(acc, axis=0)


def ActiveLearning_UncertaintySampling(X_train, X_test, y_train, y_test, n_cycles, batch_size, model, 
        model_name, method='entropy', n=10, initialtunings=False):
    """
    ...
    :param method: Optional parameter determining the measure for uncertainty. 
    ...
    clf = get_clf_after_gridsearch(X_grid, y_grid, y_train, model, model_name)
    clf.fit(X_train, y)
    ...
    for c in range(n_cycles):
        ...
        # First n cycles do a gridsearch if desired
        if c<n and initialtunings:
            clf = get_clf_after_gridsearch(X_grid, y_grid, y_train, model, model_name)
            print(c)
...
def ActiveLearning_RandomSampling(X_train, X_test, y_train, y_test, n_cycles, batch_size, 
        model, model_name, n=10, initialtunings=False):
    """
...
def ActiveLearning_QueryByCommittee(X_train, X_test, y_train, y_test, n_cycles, batch_size, model, 
        model_name, committee_size=3, n=10, initialtunings=False):
    """
...
def ActiveLearning_CostEmbeddingAL(X_train, X_test, y_train, y_test, n_cycles, batch_size, model, 
        model_name, n=10, initialtunings=False):
    """
...
def ActiveLearning_GreedySamplingX(X_train, X_test, y_train, y_test, n_cycles, batch_size, model, 
        model_name, n=10, initialtunings=False):
    """
